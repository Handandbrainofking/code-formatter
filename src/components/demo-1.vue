<template>
  <div class="device">
    <scroller class="device-scroll-view"
              :style="{ paddingBottom: fitBottomSize }"
              :show-scrollbar="false"
              :offset-accuracy="20"
              @scroll="pageScrollHandler">
      <indicator-dashboard title="实际温度"
                           :indicator="deviceNumber"
                           :status-code="deviceStatusCode"
                           :desc-group="panelDescData"
                           :base-config="baseConfig"></indicator-dashboard>
      <div class="device-action-board">
        <dof-cell avatar-icon="http://dolphin-weex-dev.msmartlife.cn/cdn/images/business/other/calorifier_icon_temperature@3x.png"
                  title="设置温度"
                  right-text="60℃"
                  ...
                  @dofCellClicked="pickTempreture"></dof-cell>
        <dof-cell avatar-icon="http://dolphin-weex-dev.msmartlife.cn/cdn/images/business/other/calorifier_icon_eco@3x.png"
                  title="节能模式"
                  desc="一键45℃，热量不浪费"
                  :icon-color="deviceThemeColorFn()"
                  :has-sub-bottom-border="true"
          ...
          <dof-switch slot="switch"
                      :type="deviceThemeColorFn('switch')"
                      :checked="switchValue"
                      :disabled="deviceDisableStatusComp"
                      @dof-change="handleSwitchChange({ type: '开关切换' })"></dof-switch>
        </dof-cell>
        <dof-cell title="E+增容"
                  desc="增容加热技术，提供更多热水"
                  :icon-color="deviceThemeColorFn()"
    ...
    <dof-actionsheet ref="modelPicker"
                     :show="modelPicker"
                     title="选择加热模式"
                     :button="buttonType"
      ...
      <div class="mode-content-slot"
           slot="content">
        <scroller class="model-scroller">
          <dof-cell v-for="(item, index) in modeList"
                    :key="index"
                    :title="item.title"
                    :desc="item.desc"
    ...
    <dof-bottom-bar :tabGroups="tabTitles"
                    @dofTabItemClicked="bottomBarOperateHandler"></dof-bottom-bar>
  </div>
</template>
